...
def index(request):
    ...
    # profitafterdeductingtax
    profitaftertax = Sale.objects.filter(date_added__year=year).aggregate(total_variable=Coalesce(
        Sum(F('grand_total')), 0.0, output_field=FloatField())).get('total_variable')
    profitaftertax = format(profitaftertax/1.18, '.2f')
